...
void dijkstra(const unordered_map<char, vector<pci>>& graph, char start, char end) {
    ...
    set<pair<int, char>> pq; // priority queue to select the minimum distance node

    // Initialize distances and previous maps to include all nodes
    ...
    if (distances[end] == INT_MAX) {
        ...
        reverse(path.begin(), path.end()); // Use the reverse function from the <algorithm> header
...
// Function to display the graph sorted by node name
...
// Function to input a new graph from the user
unordered_map<char, vector<pci>> inputGraph() {
    ...
    for (int i = 0; i < numEdges; ++i) {
        ...
        // Add edge from 'node' to 'neighbor' with weight 'weight'
...
int main() {
    ...
    while (true) {
        ...
        if (choice == 1) {
            ...
            graph = inputGraph(); // Replace current graph with user input
        } else if (choice == 4) {
            break;
        } else {
